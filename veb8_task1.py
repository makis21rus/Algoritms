from hashlib import sha1


def substr_count(string: str):
    str_length = len(string)

    is_counted = [string]
    substrings = {}

    for i in range(str_length):
        for j in range(i + 1, str_length + 1):
            subs = string[i:j]
            if subs not in is_counted and subs not in substrings:
                substrings[subs] = 0

    for patt in substrings:
        patt_length = len(patt)
        patt_hash = sha1(patt.encode("utf-8")).hexdigest()
        for k in range(str_length - patt_length + 1):
            subs_hash = sha1(string[k:k + patt_length].encode("utf-8")).hexdigest()
            if subs_hash == patt_hash:
                substrings[patt] += 1
    return substrings


s = input("Введите строку, в которой хотите узнать количество подстрок: ")
print(f'Количество подстрок: {len(substr_count(s))}')
